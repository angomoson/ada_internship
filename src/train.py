...
def main() -> None:
    ...
    if not train_names or not val_names:
        raise ValueError(
            "Need .tif files in both train and val folders. Expected:\n"
            f"  {train_root / 'pre-event'}/*.tif\n"
            f"  {train_root / 'post-event'}/*.tif\n"
            f"  {train_root / 'target'}/*.tif\n"
            f"  {val_root / 'pre-event'}/*.tif\n"
            f"  {val_root / 'post-event'}/*.tif\n"
            f"  {val_root / 'target'}/*.tif\n"
            "Run this command from the project root, or pass --train-root and --val-root explicitly."
        )
